...
from utils.weight_function import BoundaryScore_fast
...
class criterion(nn.Module):
    ...
    def __init__(self, zero_value, epoches):
        super(criterion, self).__init__()
        self.zero_value = zero_value
        self.warm_up_epoch = 10

        self.T = list(np.linspace(0, 0, self.warm_up_epoch))
        self.T.extend(list(np.linspace(0.05, 0.5, epoches - self.warm_up_epoch)))

    def get_T(self, epoch):
        return self.T[epoch]

    def forward(self, true, pred, teacher_weight, student_weight, epoch):
        loss = 0
        for idx, sample in enumerate(zip(true, pred, teacher_weight, student_weight)):
            sample_true = sample[0]
            sample_pred = sample[1]
            sample_teacher_weight = sample[2]
            sample_student_weight = sample[3]
            sample_loss = loss_func(sample_true, sample_pred)
            if sample_teacher_weight == 0 or torch.isnan(sample_teacher_weight):
                sample_teacher_weight = self.zero_value
            if sample_student_weight == 0 or torch.isnan(sample_student_weight):
                sample_student_weight = self.zero_value

            # T is temporary of distillation
            T = self.get_T(epoch)
            final_weight = 0
            try:
                final_weight = T * sample_student_weight + (1 - T) * sample_teacher_weight
                loss += final_weight * sample_loss
            except:
                final_weight = T * sample_student_weight[0] + (1 - T) * sample_teacher_weight
                loss += final_weight * sample_loss
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc='Epoch {}/{}'.format(epoch + 1, epochs), unit='img') as pbar:
            for batch in train_dataloader:
                ...
                teacher_weight = weight.cuda(non_blocking=True)
                ...
                if fp16 is True:
                    with autocast():
                        mask_pred = net(imgs)
                        student_weight = bfscore(pred=mask_pred, gt=true_masks)
                        loss = criterion_func(mask_pred, true_masks, teacher_weight, student_weight, epoch)
...
if __name__ == '__main__':
    ...
    loss_func = loss_bce
    criterion_func = criterion(zero_value=0, epoches=args.epochs)
