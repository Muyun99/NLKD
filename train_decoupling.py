...
# 利用decoupling论文算法得到idx
def decoupling(mask_pred_teacher, mask_pred_student, threshold):
...
def train_decoupling(net_student,
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc='Epoch {}/{}/val_stu:{}/val_tea:{}/idx:{}'.format(epoch + 1, epochs, val_score_student, val_score_teacher, epoch_smaple, unit='img')) as pbar:
            for batch in train_dataloader:
                ...
                sample_idx = decoupling(mask_pred_teacher, mask_pred_student, threshold=0.95)
                if len(sample_idx) == 0:
                    sample_idx = [0]
                epoch_smaple += len(sample_idx)

                # student training process
                loss_student = criterion_student(mask_pred_student, true_masks, sample_idx)
                ...
                loss_teacher = criterion_teacher(mask_pred_teacher, true_masks, sample_idx)
                ...
                pbar.set_postfix(**{'loss_student (batch)': loss_student.item(), 'loss_teacher (batch)': loss_teacher.item()})
    ...
    net_teacher.load_state_dict(torch.load(os.path.join(dir_checkpoint_now, 'teacher_bfscore_best.pth'), map_location=device))
    ...
    net_teacher.load_state_dict(torch.load(os.path.join(dir_checkpoint_now, 'teacher_miou_best.pth'), map_location=device))
